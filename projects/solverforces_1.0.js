...
Drawer = {
    ...
    drawBalls : function(context, balls) {
        ...
        for (var i = 0; i < balls.length; i++) {
            ...
            for (var k = 0; k < ball.touchinngs.length; k++) {
                ...
                context.strokeStyle = '#330';
                context.beginPath();
                context.arc(ball.x + ball.radius * touch[0], ball.y + ball.radius * touch[1], 1, 0, Math.PI * 2, true);
                context.closePath();
                context.fill();
                context.stroke();
    ...
    onMouseDownPositiontest : function(balls, posx, posy) {
        ...
        for (i = balls.length - 1; i >= 0; i--) {
            ...
            // preForce
            if (length(ball.preForceX + ball.x, ball.preForceY + ball.y, posx, posy) < 10) {
...
Ball = function(config) {
    this.x = config.x;
    this.y = config.y;
    this.radius = config.radius;
    this.mass = config.mass;
    this.fillStyle = config.color;
    this.strokeStyle = '#fe9090';
    this.preForceX = 0;
    this.preForceY = 0;
    this.postForceX = 0;
    this.postForceY = 0;
...
var App = function(canvas) {
    ...
    var paintDelegate = this.draw.bind(this);
//    var element = this.canvas;
//    (function animloop() {
//        requestAnimFrame(animloop, element);
//        paintDelegate();
//    })();
    setInterval(paintDelegate, 50);
};
App.prototype.draw = function() {
    ...
    if (this.changed) {
        ForsesSolver.clear(this.balls);
        ForsesSolver.metaSolve2(this.balls);
        this.changed = false;
    }

    Drawer.drawBalls(this.context, this.balls);
};
App.prototype.onMouseChangePosition = function (posx, posy) {
    if (this.mousemoveevent != null) {
        this.changed = true;
        this.mousemoveevent(posx, posy);
    }
};
App.prototype.onMouseDownPosition = function(posx, posy) {
    this.mousemoveevent = Drawer.onMouseDownPositiontest(this.balls, posx, posy);
};
App.prototype.onMouseUpPosition = function(posx, posy) {
    this.mousemoveevent=null;
};
...
var Input = function(app) {
    ...
    this.app.canvas.addEventListener("mousemove", this.mouseMoveHandler, false);
    this.app.canvas.addEventListener("mousedown", this.mouseDownHandler, false);
    this.app.canvas.addEventListener("mouseup", this.mouseUpHandler, false);
...
Input.prototype.mouseDown = function(e) {
    ...
    if(e.offsetX || e.offsetY){
        ...
        posy = e.clientY + document.body.scrollTop
			...
			console.log(posx,posy,e.clientX,e.clientY,document.body.scrollLeft,document.body.scrollTop,document.documentElement.scrollLeft,document.documentElement.scrollTop);
    }
    this.app.onMouseDownPosition.bind(this.app)(posx, posy);
...
Input.prototype.mouseUp = function(e) {
    ...
    this.app.onMouseUpPosition.bind(this.app)(posx, posy);
...
Input.prototype.mouseMove = function(e) {
    ...
    this.app.onMouseChangePosition(posx, posy);
}

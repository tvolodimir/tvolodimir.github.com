defineModule('tripleTownApp', ['InputExtension', 'MultiLayeredCanvas', 'Loader'], function (module, $r) {
    /**
...
//            }
        ...
        checkCollapse: function (indexX, indexY, type) {
            function found(query, x, y) {
                for (var i = 0; i < query.length; i++) {
                    if ((query[i][0] == x) && (query[i][1] == y))
                        return true;
        ...
        getUpperLevelSprite: function (type) {
            if (type == Sprite.grass)return Sprite.bush; else if (type == Sprite.bush)return Sprite.tree; else if (type == Sprite.bush_plus)return Sprite.tree; else if (type == Sprite.tree)return Sprite.hut; else if (type == Sprite.tree_plus)return Sprite.hut; else if (type == Sprite.hut)return Sprite.house; else if (type == Sprite.hut_plus)return Sprite.house; else if (type == Sprite.house)return Sprite.mansion; else if (type == Sprite.house_plus)return Sprite.mansion; else if (type == Sprite.mansion)return Sprite.castle; else if (type == Sprite.mansion_plus)return Sprite.castle; else if (type == Sprite.castle)return Sprite.flytown; else if (type == Sprite.flytown)return Sprite.flycastle;
            throw new Error(type);
        },
        getPlusSprite: function (type) {
            if (type == Sprite.grass)return Sprite.grass; else if (type == Sprite.bush)return Sprite.bush_plus; else if (type == Sprite.bush_plus)return Sprite.bush_plus; else if (type == Sprite.tree)return Sprite.tree_plus; else if (type == Sprite.tree_plus)return Sprite.tree_plus; else if (type == Sprite.hut)return Sprite.hut_plus; else if (type == Sprite.hut_plus)return Sprite.hut_plus; else if (type == Sprite.house)return Sprite.house_plus; else if (type == Sprite.house_plus)return Sprite.house_plus; else if (type == Sprite.mansion)return Sprite.mansion_plus; else if (type == Sprite.castle)return Sprite.castle; else if (type == Sprite.flytown)return Sprite.flytown; else if (type == Sprite.flycastle)return Sprite.flycastle;
            throw new Error(type);
        },
        updateCollapseItem: function (x, y, type) {
        ...
        setItemAndCollapse: function (x, y, type) {
            this.map[x][y] = type;
        ...
        setItemAndCollapseAll: function (x, y, type) {
     ...
     * @constructor
     */
    var TripleTown = function TripleTown() {
        var g = this;
        ...
        var size = [560, 560];
        this.mc = new MultiLayeredCanvas(2);
        ...
        this.mc.onDrawLayer1 = function (ctx, width, height, r, i, mc) {
            g.map.paintBackground(ctx);
            g.map.paintSprites(ctx);
            if (g.map.lastMouseCellX > -1 && g.map.lastMouseCellY > -1) {
                var dx = -40;
                var dy = -40;
                var x = g.map.lastMouseCellX;
                var y = g.map.lastMouseCellY;
                var scale = 0.8;
                var offsetx = 0.5 * 80 * (1 - scale);
                var offsety = 1.99 * 80 * (1 - scale);
                if (g.map.getMapItem(x, y) == Sprite.nothing) {
                    ctx.drawImage(g.imgs["activerectangle"].data, 0, 0, 80, 160, (x + 1) * 80 + dx, y * 80 + dy, 80, 160);
                    ctx.drawImage(g.map.getImageByType(g.activeElement).data, 0, 0, 80, 160, (x + 1) * 80 + dx + offsetx, y * 80 + dy + offsety, 80 * scale, 160 * scale);
                }
                else {
                    ctx.drawImage(g.map.getImageByType(g.activeElement).data, 0, 0, 80, 160, g.lastX - 80 + 40 * scale + offsetx, g.lastY - 120 + offsety, 80 * scale, 160 * scale);
                }
            }
            //ctx.drawImage(this.imgs["test"].data, this.map.animkey *330, 0, 330, 330, 0, 0, 560, 560);
            g.map.animkey++;
            if (g.map.animkey > 56) g.map.animkey = 0;
            g.map.activeColapseTimer++;
        };
        ...
        this.onMouseMove = function (e) {
            ...
            this.lastX = lp.x;
            this.lastY = lp.y;
            var x = Math.floor(( lp.x - 40) / 80);
            var y = Math.floor(( lp.y - 40) / 80);
